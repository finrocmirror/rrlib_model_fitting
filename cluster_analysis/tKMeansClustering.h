...
public:
  ...
  /*!
   * \brief An instantiation of std::unary_function for easier implementation of an own appropriate metric
   */
  typedef typename tKDTree::tMetric tMetric;
  ...
  tKMeansClustering(unsigned int k,
                    ...
                    tMetric metric = cDEFAULT_METRIC);
  ...
  /*!
   * \brief Get the number of clusters computed by this algorithm
   *
   * \return The number of clusters
   */
  inline size_t NumberOfClusters() const;
...
private:
   ...
   */
  typename TSample::tElement DistanceToNode(const TSample &x, const typename tKDTree::tNode &node, tMetric metric) const;
   ...
   * \param metric   The functor which computes an appropriate metric
   */
  void UpdateFromKDTreeNode(const typename tKDTree::tNode &node, tMetric metric);
   ...
   */
  template <typename TIterator>
  void Solve(TIterator samples_begin, TIterator samples_end, const tKDTree &kd_tree, const tMetric &metric);
